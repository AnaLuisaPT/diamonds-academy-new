...
import { approveInscripcion, rejectInscripcion, InscripcionDTO } from '@/lib/api'

interface PendingTabProps { inscriptions: InscripcionDTO[] }
...
export default function PendingTab({ inscriptions }: PendingTabProps) {
  ...
  const handleReject = async (insc: InscripcionDTO) => {
    await toast.promise(
      rejectInscripcion(insc.id),
      {
        loading: `Rechazando ${insc.studentName}…`,
        success:  `Solicitud de ${insc.studentName} rechazada.`,
    ...
    <Card>
      ...
      <CardContent>
        {inscriptions.length > 0 ? (
                  ...
                  <div className="flex justify-between p-4 border rounded-lg bg-yellow-50/50">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button size="sm" className="bg-green-600 text-white hover:bg-green-700"
                              onClick={() => handleApprove(insc)}>Aprobar</Button>
                      <Button size="sm" className="bg-red-600 text-white hover:bg-red-700"
                              onClick={() => handleReject(insc)}>Rechazar</Button>
                    </div>
                  </div>
                  <DialogContent className="max-w-2xl bg-white" aria-describedby={descId}>
                    <DialogHeader>
          ...
          <p className="text-center text-gray-500 py-10">¡No hay solicitudes pendientes!</p>
        )}
      </CardContent>
    </Card>
  )
